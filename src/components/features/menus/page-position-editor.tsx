...
const PagePositionEditor = ({ positions, setPositions, allCategories }: PagePositionEditorProps) => {
  const { isDark } = useTheme()
  ...
  const handleAddCategory = () => {
    ...
    if (categoryExists) {
      message.error("This category is already in the page layout")
      return
    }
    ...
    message.success("Category added to page layout")
  }
  ...
  const handleRemoveCategory = (id: string) => {
    ...
    message.success("Category removed from page layout")
  }
  ...
  return (
    <div className="position-editor">
      ...
      <div className="items-container">
        <h3 className={`section-title ${isDark ? "dark" : "light"}`}>Page Categories Order</h3>

        {positions.length === 0 ? (
          <Empty description="No categories in page layout" className="empty-state" />
        ) : (
