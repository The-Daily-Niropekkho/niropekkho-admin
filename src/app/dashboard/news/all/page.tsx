...
import { Card, Table, Button, Input, Space, Tag, Dropdown, Modal, message } from "antd"
import {
  SearchOutlined,
  PlusOutlined,
  EditOutlined,
...
export default function AllNewsPage() {
  ...
  const dataSource = Array.from({ length: 50 }, (_, i) => ({
    ...
    status: ["Published", "Draft", "Review", "Scheduled", "Archived"][Math.floor(Math.random() * 5)],
    views: Math.floor(Math.random() * 10000),
    date: new Date(Date.now() - Math.floor(Math.random() * 10000000000)).toLocaleDateString(),
    author: ["John Doe", "Jane Smith", "Robert Johnson", "Sarah Wilson", "Michael Brown"][
    ...
    <>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: isDark ? "rgba(255, 255, 255, 0.65)" : "rgba(0, 0, 0, 0.45)" }}>
          Manage and organize all news articles with advanced filtering and sorting options.
        </p>
      </div>
        ...
        <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          ...
          <Space wrap>
            ...
            <Button type="primary" icon={<PlusOutlined />}>
              <Link href="/dashboard/news/create" style={{ color: "inherit" }}>
                Create News
              </Link>
            </Button>
          </Space>
