...
export default function PollsPage() {
    ...
    const [pollImage, setPollImage] = useState<TFileDocument | undefined>(
        undefined
    );
    ...
    const handleEdit = (record: Poll) => {
        ...
        setPollImage(record.banner_image ? record.banner_image : undefined);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text: string,record: Poll) => (
                <Link href={`${config.host_front}/opinion-poll/${record?.id}/${record.slug}`} style={{ maxWidth: "300px", display: "block" }}>{text}</Link>
            ),
        },
        {
            title: "Description",
            ...
            key: "description",
            render: (text: string) =>
                text ? <h5 style={{ maxWidth: 300 }}>{text}</h5> : "-",
        },
        {
            title: "Status",
            dataIndex: "status",
